...
window.addEventListener('DOMContentLoaded', resetLogin);
...
tokenInputs.forEach((input, index) => {
    ...
    input.addEventListener('keydown', (e) => {
        if (e.key === 'Backspace' && e.target.value === '' && index > 0) tokenInputs[index - 1].focus();
    });
    input.addEventListener('paste', (e) => {
        e.preventDefault();
        const paste = e.clipboardData.getData('text');
        if (/^\d{6}$/.test(paste)) {
            tokenInputs.forEach((ti, i) => ti.value = paste[i]);
...
if (btnUnico) {
    btnUnico.addEventListener('click', async (e) => {
        e.preventDefault();
        switch (etapaAtual) {
            case 1: await validarEmail(); break;
            case 2: await validarSenha(); break;
            case 3: await validarToken(); break;
            default: exibirMensagem('Erro de estado do formulário. Recarregue a página');
        }
    });
}
